...
describe("runAgentTurn", () => {
    it("responde directo cuando el modelo no pide ninguna tool", async () => {
        ...
        const outcome = await (0, aiAgent_1.runAgentTurn)("user-1", "hola", { gemini, readRecentCheckIns, saveAlert });
    ...
    it("lee el historial reciente del usuario autenticado cuando el modelo pide la tool", async () => {
        ...
        const saveAlert = jest.fn();
        const outcome = await (0, aiAgent_1.runAgentTurn)("user-1", "no se que hacer", {
            gemini,
            readRecentCheckIns,
            saveAlert,
        });
    ...
    it("guarda una alerta real cuando el modelo determina riesgo rojo", async () => {
        ...
        const outcome = await (0, aiAgent_1.runAgentTurn)("user-1", "quiero consumir ahora", {
            gemini,
            readRecentCheckIns,
            saveAlert,
        });
        expect(saveAlert).toHaveBeenCalledWith(expect.objectContaining({ userId: "user-1", riskLevel: "ROJO", message: "Activar protocolo de emergencia" }));
    ...
    it("nunca permite que el modelo pida el historial de otro usuario: el uid siempre es el de la sesion", async () => {
        ...
        const saveAlert = jest.fn();
        await (0, aiAgent_1.runAgentTurn)("user-1", "hola", { gemini, readRecentCheckIns, saveAlert });
